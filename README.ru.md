...
**Генератор караоке-видео из музыкальных клипов и аудиофайлов**

CLI-приложение для macOS, которое принимает на вход музыкальное видео или аудиофайл и автоматически превращает его в караоке: отделяет вокал от инструментала, распознаёт текст песни с точными таймингами по словам, накладывает синхронизированные субтитры с подсветкой текущего слова и выдаёт готовый MP4-файл. Для аудиофайлов автоматически генерируется видео с чёрным фоном.
...
- **Караоке-субтитры** — ASS-формат с тегами `\kf` для плавной заливки слов слева направо, чередующееся двухстрочное отображение (текст не перепрыгивает между строками)
...
- **Поддержка аудиофайлов** — MP3, FLAC, WAV, OGG, M4A, AAC, WMA, Opus, AIFF — генерирует видео с чёрным фоном
- **Гибкая настройка** — цвета, шрифт, позиция субтитров, профили качества, разрешение видео
...
|------|-----|-------------|----------|
| `INPUT_FILE` | path | — | Путь к медиафайлу: видео или аудио (обязательный) |
...
| `--font-size` | int | `64` | Размер шрифта (px) |
...
Всегда отображаются две строки: чётные сегменты на верхней строке, нечётные на нижней. Пока поётся одна строка, на другой показан следующий сегмент. Когда пение переходит на следующую строку, предыдущая обновляется новым текстом — текст не перепрыгивает между строками.

```ass
Dialogue: 0,0:01:05.00,0:01:10.00,Line1,,0,0,0,,{\kf50}Я {\kf30}помню {\kf40}чудное {\kf60}мгновенье
Dialogue: 0,0:01:05.00,0:01:10.00,Line2,,0,0,0,,Передо мной явилась ты
```
...
**Входные аудио:** `.mp3`, `.wav`, `.flac`, `.ogg`, `.m4a`, `.aac`, `.wma`, `.opus`, `.aiff`
